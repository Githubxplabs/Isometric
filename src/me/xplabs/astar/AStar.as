package me.xplabs.astar
{
	...
	public class AStar
	{
		...
		private var straight:Number;
		private var _heuristicType:int;
		//====================================
		//	Constructor
		//====================================
		/**
		...
		public function AStar(p_mapTileModel : IMapTileModel, p_maxTry : int = 500)
		{
			...
			_heuristicType = m_mapTileModel.heuristicType;
		}
		//====================================
		//	Properties
		//====================================
		...
		public function find(p_startX : int, p_startY : int, p_endX : int, p_endY : int) : Array
		{
			...
			while (this.m_openCount > 0)
			{
				...
				for each (var note : Array in aroundNotes)
				{
					...
					//var dx:Number = p_endX - note[0];
					//var dy:Number = p_endY - note[1];
					//score = cost + (Math.abs(dx) + Math.abs(dy)) * COST_STRAIGHT;
					score = heuristic(note[0], note[1], p_endX, p_endY, cost);
		 ...
		 * @return
		 */
		private function heuristic(sx:Number, sy:Number, ex:Number, ey:Number, cost:Number = 0):Number
		{
			var diag:Number;
			var dx:Number;
			var dy:Number;
			...
			switch(_heuristicType)
			{
				case AstarHeuristic.diagonal:
					...
					diag = Math.min(dx, dy);
					straight = dx + dy;
					score =  diag * Math.SQRT2 + COST_STRAIGHT * (straight - 2 * diag);
					break;
				case AstarHeuristic.euclidian:
					dx = ex - sx;
					dy = ey - sy;
					score = Math.sqrt(dx * dx + dy * dy) * COST_STRAIGHT;
					break;
